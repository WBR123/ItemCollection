// JavaScript Document
        ...
        var box  = document.getElementById("column_near");  //   获得大盒子
       ...
       // 4. 定时器部分  难点
        ...
        timer = setInterval(autoplay,5000);   // 每隔5s 调用一次 autoplay
        function autoplay() {
            key++;   // key == 1  先 ++
            console.log(key); //  不能超过4
            if(key > ulLis.length - 1)
            {
               // alert('停下');
                ul.style.left = 0;
                key = 1;  // 因为第4张就是第一张，我们已经播放完毕了， 下一次播放第2张
                // 第2张的索引号是1
            }
        ...
        box.onmouseout = function() {
            timer = setInterval(autoplay,5000);  // 一定这么开
        }
